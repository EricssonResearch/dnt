...
ip route add 10.0.200.0/24 via 192.168.55.2 dev swp0
ip route add 10.0.200.0/24 via 192.168.66.2 dev swp1 metric 5
...
ip addr add 10.0.100.1/24 dev ovs0
ovs-vsctl set bridge ovs0 protocols=OpenFlow10,OpenFlow11,OpenFlow12,OpenFlow13
ovs-ofctl -O OpenFlow13 add-flow ovs0 in_port=1,dl_type=0x800,nw_src=10.0.100.11,nw_dst=10.0.200.22,actions=output=2
