# Scenario #3: R2DTWO IPv4 over DetNet
...
That means the IP packets from the talker will be encapsulated into MPLS DetNet CW and routed toward the PEF.
The PEF handles the duplicates, then after decapsulating the packet __until the IPv4 header__ sent by the talker, it will send the packet to the listener over a native IP interface.
...
Also, most of the configuration is similar to the TSN over DetNet case (Scenario #2).

One of the main differences is now we have a new egress interface: `uni_ip`.
...
The `[objects]` section is the same as we seen in Scenario #2.
