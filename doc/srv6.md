...
* bel2: internal address for the vrf1 interface. This will be used as source IP for the outer IPv6 packet with the PREOF SID as destination.
* bel3: internal address for the ve1 interface. This will be used as gateway for the PREOF.SID prefix range.
* bel4: internal address for the ve2 interface. This is only needed for R2DTWO IP interface.
...
* R2DTWO uses a vrf interface to send IP packets to NNI interfaces. The choice for VRF type interface was that it behaves as an IP interface, not requiring ARP/NDP support. SRv6 routing entries will be used to direct traffic from the VRF interface towards the tunnels. The VRF interface must also have an IPv6 address, from range `be12`. This has local meaning, a routing entry will use it to route the /64 PREOF SID range through this interface to SRv6 tunnels (H.Encap).
* R2DTWO uses an additional `veth` interface pair (`ve1` and `ve2`) to receive the decapsulated IP packets. Interface `ve1` is configured with an address from range `be13`. To match the PREOF SIDs a /64 prefix network route is added to the routing table with a next hop in the `be13` range, for example `be13::99`. Therefore all traffic exiting the End.DT6 function will be routed through the `ve1` interface. Since `ve1` is an Ethernet type interface, a static neighbor entry is needed for the next hop `be13::99`. This is needed to resolve the MAC address of the next hop, and also helps to avoid local L2 loops as this MAC address is not a local address. At the same time, the R2DTWO listens on the `ve2` interface at IP level. The R2DTWO `ip` interface type requires that `ve2` has an IP address. To fulfill this, `ve2` is equipped with an address from `be14` range. (This IP is not used, so any local IP will do here.)
...
fd00:be13::/64 proto kernel metric 256 pref medium
