...
The R2DTWO support for SRv6 relies on the Linux SRv6 functions. As currently the Linux SRv6 implementation does not support DetNet, we use an outer IPv6 encapsulation which uses a DetNet SID as destination address. This IPv6 encapsulated packet will be directed into a predefined SRv6 tunnel.
```
┌─────────────────────────────────────┐                                  
...
┌──────────────────────┬─────────────────────────────────────┐                                  
│IPv6 header, dest=SID │ Original packet (IPv6, IPv4 or TSN) │    R2DTWO                        
└──────────────────────┴─────────────────────────────────────┘                                  

┌──────────────────────┬──────────────────────┬─────────────────────────────────────┐                                
│IPv6 header + SRH     │IPv6 header, dest=SID │ Original packet (IPv6, IPv4 or TSN) │    Linux SRv6 encap seg6            
└──────────────────────┴──────────────────────┴─────────────────────────────────────┘                                 
```
To support the DetNet SID structure, the following header fields have been added to the IPv6 header:
...
└──────────────┴──────────────┴────────────────────────────────────────┘                       
```
At the UNI interface, the incoming traffic will be identified and directed to R2DTWO. R2DTWO will encapsulate the incoming packet (which can be IPv6, IPv4 or TSN), and sets the destination DetNet SID. The encapsulated packet will be routed to an SRv6 encap LWT, which encapsulates again the packet within a new IPv6 header containing the SRH.
At the egress node, incoming packets will be directed to an End.DT6 termination, which decapsulates the SRv6 headers. The decapsulated IPv6 packet containing the DetNet SID will be routed to the R2DTWO. The R2DTWO performs identification, elimination, and decapsulates the original packet to send out on the UNI.
...
alfa    ║                                    ║  alfa           alfa   ║                                    ║  alfa
        ║                                    ║                        ║                                    ║      
        ║    SID: bela                       ║                        ║    SID: bela                       ║      
        ║    sr0: fade               Linux   ║                        ║    sr0: fade                Linux  ║      s
        ╚════════════════════════════════════╝                        ╚════════════════════════════════════╝      
```
The operation is the following: the incoming IP traffic from talker t1 is identified by the TC filter, and it is redirected to the veth interface. As the `t1r2` interface has IP address, it handles the ARP/NDP messages locally. The R2DTWO listens on the veth interface, and receives the packets. (Note that in case of TSN over SRv6 the veth interface pair is not needed, and also no IP is needed on the UNI interfac.e) After flow identification, R2DTWO adds the outer IPv6 header which has the destination address the DetNet SID containing the `locator`, `func`, `flowid` and `seq` fields. The encapsulated packet is sent on the VRF interface.
