...
def is_recent_tshark():
    ...
    if ret.stdout:
        if "rtag.seqno" in ret.stdout:
            return 1
        elif "tshark -G <report>" in ret.stdout:
            return 1
    return 0
...
def ping():
    try:
        ...
        exec_bg("../dnt sequence/dntbr0.ini")
        exec_bg("../dnt sequence/dntbr1.ini")
        time.sleep(1)

        num_pings = 1000
        pingcmd = exec_fg(f"ping -I to_dntbr0 10.0.0.2 -i 0.0001 -c {num_pings}", timeout=5.0)
        ...
        num_pings = 70000
        pingcmd = exec_fg(f"ping -I to_dntbr0 10.0.0.2 -i 0.0001 -c {num_pings}", timeout=10.0)
        if pingcmd.stdout and f", {num_pings} received," not in pingcmd.stdout:
            print(pingcmd.stdout)
            return 0
    except:
...
def reset():
    try:
        ...
        r2 = exec_bg("../dnt sequence/dntbr0.ini")
        exec_bg("../dnt sequence/dntbr1.ini")
        time.sleep(2)

        num_pings = 10
        pingcmd = exec_fg(f"ping -I to_dntbr0 10.0.0.2 -i 0.0001 -c {num_pings} -W 4", timeout=5.0)
        if pingcmd.stdout and f", {num_pings} received," not in pingcmd.stdout:
            print(pingcmd.stdout)
            return 0
...
def flapping():
    try:
        ...
        num_pings = 500
        ping = exec_bg(f"ping -I to_dntbr0 10.0.0.2 -i 0.01 -c {num_pings} -W 10", out=OUT_PIPE)
        for cmd in ["down", "up"] * 5:
            time.sleep(0.1)
            exec_fg(f"ip link set dev dntbr1_nni0 {cmd}")
        for cmd in ["down", "up"] * 5:
            time.sleep(0.1)
            exec_fg(f"ip link set dev dntbr1_nni1 {cmd}")
...
def flapping_bad():
    ...
    exec_bg("../dnt sequence/dntbr1_norcvy.ini")
    if flapping() == True:
        return 0
    return 1
...
def resetonly_bad():
    ...
    try:
        br0 = exec_bg("../dnt sequence/dntbr0_resetonly.ini")
        exec_bg("../dnt sequence/dntbr1.ini")
        time.sleep(1)
        num_pings = 20
        pingcmd = exec_bg(f"ping -I to_dntbr0 10.0.0.2 -i 0.1 -c {num_pings}", out=OUT_PIPE)
        time.sleep(1)
        # Reset the seq generators of dntbr0 instance with SIGUSR1
...
def resetonly_good():
    ...
    try:
        br0 = exec_bg("../dnt sequence/dntbr0_resetonly.ini")
        exec_bg("../dnt sequence/dntbr1_resetonly.ini")
        time.sleep(1)
        num_pings = 20
        pingcmd = exec_bg(f"ping -I to_dntbr0 10.0.0.2 -i 0.1 -c {num_pings}", out=OUT_PIPE)
        time.sleep(1)
...
def seamless_bad():
    ...
    try:
        br0 = exec_bg("../dnt sequence/dntbr0_init.ini")
        exec_bg("../dnt sequence/dntbr1.ini")
        time.sleep(1)
        num_pings = 20
        pingcmd = exec_bg(f"ping -I to_dntbr0 10.0.0.2 -i 0.1 -c {num_pings}", out=OUT_PIPE)
...
def seamless_good():
    ...
    try:
        if is_recent_tshark():
            tsharkcmd = "tshark -l -O 'ieee8021cb' -T fields -e rtag.seqno -i dntbr0_nni0"
            print("new tshark")
            seq_base = 10 #new tshark using decimal print format
        else:
            print("old tshark")
            tsharkcmd = "tshark -l -O 'ieee8021cb' -e ieee8021cb.seq -T fields -i dntbr0_nni0"
            seq_base = 16 #old tshark using hexa print format
        br0 = exec_bg("../dnt sequence/dntbr0_init.ini")
        tshark = exec_bg(tsharkcmd, out=OUT_PIPE)
        exec_bg("../dnt sequence/dntbr1_init.ini")
        time.sleep(1)
        num_pings = 30
        pingcmd = exec_bg(f"ping -I to_dntbr0 10.0.0.2 -i 0.1 -c {num_pings}", out=OUT_PIPE)
        time.sleep(1)
...
def match_good():
    ...
    try:
        exec_bg("../dnt sequence/dntbr0_match.ini", out=OUT_NONE)
        exec_bg("../dnt sequence/dntbr1_match.ini", out=OUT_NONE)
        time.sleep(1)
        pingcmd = exec_fg(f"ping -I to_dntbr0 10.0.0.2 -A -c 5 -W 5", timeout=3.0)
        num_pings = 500
        pingcmd = exec_fg(f"ping -I to_dntbr0 10.0.0.2 -A -c {num_pings} -W 5", timeout=3.0)
        if pingcmd.stdout and f", {num_pings} received," not in pingcmd.stdout:
            print(pingcmd.stdout)
            return 0
    except:
...
def main():
    print("DNT sequence generator and recovery test\n")
    try:
        create_ifaces()
        config_ifaces()
