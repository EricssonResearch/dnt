...
This directory contains python library and examples to receive and handle R2DTWO notifications.
...
The *json_udp_receiver* is a receiver that displays the messages _as is_, without reassembling the fragments.
However, elimination of multiple messages is performed.

The *notification_receiver* is a python library that performs both the elimination of duplicates and reassembly of the messages.

The *multipart_json_udp_receiver* is a receiver based on the *notification_receiver* library that receives the fragments and displays the reassembled messages.

## Usage

The  *notification_receiver* provides a *NotificationReceiver* class, which implements all the required functionality. To use the class, first we need to instantiate a receiver:
