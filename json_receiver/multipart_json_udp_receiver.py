...
while True:
    ...
    try:
        ...
        if ( frag_id == None  or frag_id == "1/1" ): # not multipart message
            ...
            # mpart_collector[hostname][idx] = int(frag_id[0:frag_id.find('/')])
            # mpart_collector[hostname][items] = int(frag_id[frag_id.find('/')+1:])
            ...
            if (not omit_fragment):
                ...
                # print ("hostname= ", hostname, "idx= ", idx, " items= ", items, "seq= ", seq_num, "last_seq= ", mpart_collector[hostname]["last_seqnum"], "frag_count= ", mpart_collector[hostname]["frag_count"])
